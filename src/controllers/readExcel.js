...
exports.readFileExcel = async (req, res) => {
    try {
        ...
        for (let row = 4; row < data.length; row++) {
            ...
            allData[data[row][`__EMPTY_${2}`]] = dateWork;


        }
        const pathfile = path.resolve(__dirname, "..", "..", "upload", "bangcong.xlsx");
        if (fs.existsSync(pathfile)) {
            fs.unlinkSync(pathfile);
        }
        return res.jsonp(allData);
    } catch (error) {
        console.log(error);
    }
}
